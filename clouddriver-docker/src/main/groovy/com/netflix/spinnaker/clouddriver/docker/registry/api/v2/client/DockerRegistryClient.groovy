...
class DockerRegistryClient {
  ...
  interface DockerRegistryService {
    ...
    Response getV2Manifest(@Path(value="name", encode=false) String name, @Path(value="reference", encode=false) String reference, @Header("Authorization") String token, @Header("User-Agent") String agent)
  ...
  public String getConfigDigest(String name, String tag) {
    def response = getV2Manifest(name, tag)
  ...
  private getV2Manifest(String name, String tag) {
    request({
      registryService.getV2Manifest(name, tag, tokenService.basicAuthHeader, userAgent)
    }, { token ->
      registryService.getV2Manifest(name, tag, token, userAgent)
    }, name)
  }
